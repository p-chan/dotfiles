...
`pnpm install` 時に peer dependencies のエラーが出る場合、根本的な解決（依存関係の追加など）が困難なときは `pnpm-workspace.yaml` で無効化することをユーザーに確認する（[references/config.md](../references/config.md)）。

### hoisting が必要なパッケージ

pnpm はデフォルトで依存関係を厳格に解決するため、`require()` でパスを直接解決する古いパッケージが動作しないことがある。
`publicHoistPattern` で対応できる場合はそちらを優先し、それでも解決できない場合のみ `shamefullyHoist: true` をユーザーに提案する（[references/config.md](../references/config.md)）。
