...
description: プロジェクトに pnpm 環境をセットアップします。packageManager フィールドの設定、.npmrc の設定、既存パッケージマネージャーからの移行、CI の pnpm セットアップを行います。pnpm のセットアップを求められたときに使用してください。
---
...
[references/npmrc.md](references/npmrc.md) に従う。
...
エラーが出た場合は内容を確認し、必要に応じて `.npmrc` の設定を調整する（詳細は [references/npmrc.md](references/npmrc.md)）。
