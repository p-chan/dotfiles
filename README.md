...
path in `settings.json` as `$HOME` and preserves JSON sort normalization, while
the smudge filter restores the local absolute path before Herdr runs. This keeps
the settings portable without interfering with Herdr integration updates.
